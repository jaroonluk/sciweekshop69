...
CREATE TABLE event_rounds (
  ...
  ask_high_power TINYINT(1) NOT NULL DEFAULT 0 COMMENT 'ถามการใช้ไฟฟ้ากำลังสูงในใบสมัคร',
  ask_ice_bucket TINYINT(1) NOT NULL DEFAULT 0 COMMENT 'ถามการใช้ถังน้ำแข็งในใบสมัคร',
  apply_flow VARCHAR(32) NOT NULL DEFAULT 'zone_then_category' COMMENT 'zone_then_category | category_only',
  notes TEXT NULL,
  created_at DATETIME NOT NULL DEFAULT CURRENT_TIMESTAMP,
  updated_at DATETIME NOT NULL DEFAULT CURRENT_TIMESTAMP ON UPDATE CURRENT_TIMESTAMP,
  PRIMARY KEY (id),
...
CREATE TABLE applicants (
  ...
  need_high_power TINYINT(1) NULL COMMENT '1=ใช้ไฟฟ้ากำลังสูง 0=ไม่ใช้ NULL=ไม่ถาม',
  ice_bucket_count SMALLINT UNSIGNED NULL COMMENT 'จำนวนถังน้ำแข็ง NULL=ไม่ถาม 0=ไม่ใช้',
